/* ===========================================================
   TrainPro — painel do assinante (Supabase: multi-tenant real)
   Cada login = um "business" isolado por Row Level Security.
   =========================================================== */

const SEGMENTS = {
  personal:     { label: "Personal Trainer",     services: [["Avaliação Física",50,100],["Treino Individual",60,90],["Treino em Dupla",60,70],["Pacote Mensal (12 sessões)",60,720]] },
  academia:     { label: "Academia / Studio",    services: [["Aula Experimental",45,0],["Plano Mensal",0,150],["Personal na Academia",60,90]] },
  outro:        { label: "Outro segmento",       services: [["Sessão",60,100],["Retorno",30,60]] }
};
...
async function renderSponsorBanner(){
  const { data } = await supabaseClient.from("patrocinadores").select("*").eq("produto", "trainpro").eq("ativo", true).limit(1).maybeSingle();
...
async function iniciarAssinatura(plano){
  try{
    const resp = await fetch(`${BACKEND_URL}/api/assinatura/criar`, {
      ...
      body: JSON.stringify({ business_id: BUSINESS.id, email: CURRENT_USER.email, plano, produto: "trainpro" })
    });
...
function renderSubscriptionGate(){
  ...
  const msg = trialExpirado()
    ? "Esperamos que tenha gostado! Escolha um plano abaixo pra continuar usando o TrainPro."
    : `Sua assinatura do TrainPro está <strong>${BUSINESS.subscription_status}</strong>. Escolha um plano abaixo para voltar a usar o app.`;
...
async function loadOrCreateBusiness(){
  ...
  if(!biz){
    const slug = "profissional-" + Math.random().toString(36).slice(2,8);
    const { data: newBiz, error } = await supabaseClient.from("businesses")
      .insert({ owner_id: CURRENT_USER.id, slug, name: "Meu Atendimento", segment: "personal", brand_color: "#FF4D8D" })
      .select().single();
...
/* ---------------- THEME / BRAND ---------------- */
function applyBrand(){
  document.documentElement.setAttribute("data-theme", BUSINESS.fundo_estilo === "grafite" ? "dark" : "light");
  const cor = BUSINESS.brand_color || "#FF4D8D";
  document.documentElement.style.setProperty("--accent", cor);
  document.documentElement.style.setProperty("--accent-ink", contrastInk(cor));
  document.getElementById("brandName").textContent = BUSINESS.name || "TrainPro";
...
function fillConfigForm(){
  ...
  document.getElementById("cfgCor").value = BUSINESS.brand_color || "#FF4D8D";
  document.getElementById("cfgFundo").value = BUSINESS.fundo_estilo || "branco";

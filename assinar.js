...
document.getElementById("cadastroForm").addEventListener("submit", async (e)=>{
  ...
  try{
    ...
    const { data: business, error: bizError } = await supabaseClient.from("businesses")
      .insert({ owner_id: session.user.id, slug, name: nomeNegocio || "Meu Negócio", segment: "personal", brand_color: "#FF4D8D", subscription_plan: planoSelecionado })
      .select().single();
    ...
    const resp = await fetch(`${BACKEND_URL}/api/assinatura/criar`, {
      ...
      body: JSON.stringify({ business_id: business.id, email, plano: planoSelecionado, produto: "trainpro" })
    });
